...
function shouldSkipNode(node) {
    ...
    if (parent) {
        ...
        const problemDesc = parent.closest('[data-track-load="description_content"], .elfjS, [class*="description"]');
...
function translateTextNode(node) {
    ...
    if (node.parentElement && node.parentElement.closest('[data-translated-welcome]')) {
        if (val.includes("Welcome to LeetCode!")) {
...
function handleSpecialUIPatterns(element) {
    ...
    if (originalText.includes("Welcome to LeetCode!")) {
    ...
    if (text.startsWith("Completed a daily challenge for") && text.length < 150) {
...
function translateNode(node) {
    if (node.nodeType === 3 && node.nodeValue && /Consistency is key,\s*see you tomorrow!/i.test(node.nodeValue)) {
        node.nodeValue = node.nodeValue.replace(/Consistency is key,\s*see you tomorrow!/i, "꾸준함이 핵심입니다. 내일 만나요!");
        return;
    }
    ...
    if (node.nodeType === 3) {
        const isConsistencyText = node.nodeValue && /Consistency is key,\s*see you tomorrow!/i.test(node.nodeValue);
        if (isConsistencyText || !shouldSkipNode(node)) translateTextNode(node);
    } else if (node.nodeType === 1) {
        const hasConsistencyText = node.textContent && /Consistency is key,\s*see you tomorrow!/i.test(node.textContent);
